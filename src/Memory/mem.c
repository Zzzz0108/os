...
static int execute_fifo_replacement() {
    int victim_frame = -1;
    uint32_t oldest_install_time = 0xFFFFFFFF;

    // 扫描所有物理页框，找到装入时间最早的页
    for (int i = 0; i < total_phys_pages; i++) {
        PTE* pte = frame_reverse_map[i];
        if (pte != NULL && fifo_install_time[i] < oldest_install_time) {
            oldest_install_time = fifo_install_time[i];
            victim_frame = i;
        }
    }
...
static int execute_clock_replacement() {
    ...
    // 循环扫描，找到 access=0 的页
    while (frames_scanned < total_phys_pages) {
        PTE* pte = frame_reverse_map[clock_ptr];
...
static int execute_replacement() {
    switch (current_algorithm) {
        case ALGORITHM_LRU:
            return execute_lru_replacement();
        case ALGORITHM_FIFO:
            return execute_fifo_replacement();
        case ALGORITHM_CLOCK:
            return execute_clock_replacement();
        default:
            return execute_lru_replacement();
    }
}
...
static int handle_page_fault(PTE* pte) {
    ...
    if (current_algorithm == ALGORITHM_FIFO) {
        fifo_install_time[frame] = ++global_time;
    }
...
int init_memory_system(int num_phys_pages) {
    return init_memory_system_with_algorithm(num_phys_pages, ALGORITHM_LRU);
}
...
void print_mem_status(void) {
    ...
    switch (current_algorithm) {
        ...
        case ALGORITHM_FIFO:
            ...
            break;
        case ALGORITHM_CLOCK:
            algo_name = "CLOCK (Clock Algorithm)";
            break;
    }
